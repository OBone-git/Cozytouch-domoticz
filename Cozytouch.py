...
def gestion_switch_selector_domoticz (cozytouch_mode_actual, url_device, nom_device, idx_switch_domoticz,state_cozytouch_on_off='no',
                                                 ...
                                                 command_Manual_activate = False, setting_command_Manual=u'setDerogatedMode', setting_parameter_Manual_on=u'on',setting_parameter_Manual_off=u'off',
    ...
    if cozytouch_mode_actual != domoticz_mode_actual and domoticz_mode_old != 'init':
        ...
        if domoticz_mode_actual != domoticz_mode_old :
            ...
            # Options de commande Manuel :
            if command_Manual_activate :
                if domoticz_switch_actual == level_10 :
                    # Envoi de la commande Manuel (associé par défaut au level_10)
                    cozytouch_POST(url_device,setting_command_Manual,setting_parameter_Manual_on)
                else :
...
def maj_device(data,name,p,x):
    ...
    if name == dict_cozytouch_devtypes.get(u'PAC Heatpump') :
        ...
        print (value_by_name(data,x,u'io:PassAPCOperatingModeState'))
