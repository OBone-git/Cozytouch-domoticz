...
# TEST
dict_cozytouch_devtypes['PAC HeatPump']='io:AtlanticPassAPCHeatPumpMainComponent'
...
def decouverte_devices():
    ...
    if not save_devices : # si la liste est vide on passe à la création des devices
        ...
        for a in data[u'setup'][u'devices']:
            ...
            elif name == dict_cozytouch_devtypes.get(u'PAC HeatPump'):
...
def ajout_PAC_HeatPump  (idx,liste,url,x,label):
    ...
    nom_mesure = u'Outdoor T°C'
    PAC_HeatPump [u'idx_mesure_temp_ext']= domoticz_add_virtual_device(idx,80,nom_mesure)

    # Création Mesure température Intérieur :
    nom_mesure = u'Indoor T°C'
    PAC_HeatPump [u'idx_mesure_temp_int']= domoticz_add_virtual_device(idx,80,nom_mesure)
...
def maj_device(data,name,p,x):
    ...
    if name == dict_cozytouch_devtypes.get(u'PAC Heatpump') :
        ...
        # Mesure température extérieur :
        domoticz_write_device_analog(value_by_name(data,(x+1),u'core:TemperatureState'),classe.get(u'idx_mesure_temp_ext'))
        
        # Mesure température intérieur :
        domoticz_write_device_analog(value_by_name(data,(x+5),u'core:TemperatureState'),classe.get(u'idx_mesure_temp_int'))
    ...
    if name == dict_cozytouch_devtypes.get('PAC zone component') :
        ...
        # Gestion switch sélecteur : Prise en compte du mode de fonctionnement "Derogation"
        ...
        return_switch = gestion_consigne(u'Manuel',classe.get(u'url'),classe.get(u'nom'),classe.get(u'idx_cons_temp_manu'),value_by_name(data,x,u'core:DerogatedTargetTemperatureState'),setting_consigne_zone)
        # Evaluation du retour de la fonction, si cas n°1, on a envoyé un changement vers Cozytouch
        if return_switch == 1 :
            # Application d'une durée fixe de dérogation ainsi qu'une commande d'activaton de dérogation :
            # Commande "setDerogationTime" / State : io:DerogationRemainingTimeState (480 mins par défaut)
            derog_time = 480
            cozytouch_POST(classe.get(u'url'),u'setDerogationTime',derog_time)
            # Commande "setDerogationOnOffState" à on pour activer le mode dérogation
