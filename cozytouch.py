...
domoticz_server = "http://" + ma_config.domoticz_ip + ":" + ma_config.domoticz_port
...
url_domoticz = domoticz_server + "/json.htm?type="
...
def domoticz_rename_hardware(idx, nom):
    ...
    myurl = (
        url_domoticz
        + "command&param=updatehardware&htype=15"
        + "&enabled=true"
        + "&idx="
        + idx
        + "&name="
...
def domoticz_add_virtual_device(hwidx, typ, nom, option="none"):
    ...
    if debug == 555:  # remplissage manuel
        ...
        myurl = (
            url_domoticz
            + "command&param=createvirtualsensor&idx="
            + hwidx
            + "&sensorname="
            + nom
            + "+&sensortype="
                ...
                while True:
                    try:
                        ...
                        if a == save_idx:
                            ...
                            versionactuelle = data["result"][y]["Name"].split("V")
                            ...
                            if data["result"][y]["Name"] == "Cozytouch_V" + str(
                                version
                            ):
                                ...
                                if debug:
                                    print(
                                        "idx hardware cozytouch dans domoticz : "
                                        + str(a)
                                    )
                                break
                            elif (
                                int(versionactuellemajor)
                                >= 5 & int(versionactuellemajor)
                                == int(versionciblemajor)
                            ):
                                # si update de version mineure alors mise à jour du hardware dans domoticz (valable à partir de la version majeure 5.xx)
                                if debug:
                                    print(
                                        "version actuelle : "
                                        + str(versionactuellemajor)
                                        + "."
                                        + str(versionactuelleminor)
                                        + " version cible : "
                                        + str(versionciblemajor)
                                        + "."
                                        + str(versioncibleminor)
                                    )
                                reset = False
                                nouveaunom = "Cozytouch_V" + str(version)
                                if debug:
                                    print(
                                        "domoticz_rename_hardware : "
                                        + str(save_idx)
                                        + " "
                                        + str(nouveaunom)
                                    )
                                ...
                                if debug:
                                    print("Update de version majeure")
                                reset = True
                                break
                        elif a != save_idx:
                            y += 1
                        ...
                        print(
                            "**** Exception a la lecture de la configuration cozytouch ****"
                        )
                        reset = True
                        break
            else:
...
def Add_DHWP_THERM(idx, liste, url, x, label, name):
    ...
    # Création Compteur d'énergie :
    nom_compteur = "Conso " + nom
    DHWP_THERM["idx_compteur"] = domoticz_add_virtual_device(
        idx, 113, nom_compteur
    )  # non utilisé ensuite
    # Switch selecteur :
    nom_switch = "Mode " + nom
    ...
    if (
        ...
        if cons_device_abais_eco > 0:
            ...
            if cons_domoticz_abais_eco < 2:
                ...
                print(
                    "consigne abaissement eco Domoticz " + str(cons_domoticz_abais_eco)
                )
            cozytouch_POST(
...
print(
    ("¤¤¤¤ Demarrage script cozytouch " + str(version) + " (debug :" + str(debug) + ")")
)
...
if not (pvma == 2 and pvmi == 7):
    print(
        "!!!! Echec test version python, ce script nécessite python 2.7.x (idealement 2.7.15)"
    )
